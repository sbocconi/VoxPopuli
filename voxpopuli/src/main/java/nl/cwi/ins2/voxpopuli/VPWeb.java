...
public class VPWeb {
	...
	private String CreateRhetoric(SMILMedia theSMILMedia, RuleInstance theRuleInstance, DataContainer theDataContainer,
		...
		if (Opinions == null || Opinions.length != 1 || Interviewees == null || Interviewees.length != 1) {
			return null;
		}
		...
		try {
			...
			String queryString = "select Interview, PosDesc, PartDesc  " + "from "
					+ "{Interview} rdf:type {VoxPopuli:InterviewSegment}, " + "{Interview} VoxPopuli:opinion {<"
					+ Opinions[0] + ">}, " + "{Interview} VoxPopuli:partecipant {<" + Interviewees[0] + ">}, " + "{<"
							...
							if (theDataContainer.InterviewsArray[ii].Arguments != null) {
								for (int iii = 0; iii < theDataContainer.InterviewsArray[ii].Arguments.length; iii++) {
									...
									theRuleInstance.AugmentStructure(Strategy, false, false, true, augmented,
											theDataContainer.InterviewsArray[ii].Arguments[iii], Voices);
									// irrelevant here
	...
	public String[][] RetrieveInterviewees(DataContainer theDataContainer, String[] Questions, String[] Opinions)
			throws Exception {
	...
	public String SetRules(String[][] Classes, String[][] parameters1, String[][] parameters2, String[] concepts1,
			String[] concepts2, String[] questions, String[] interviewees, String[] opinions, String[] interviews,
	...
	/*
	 * public boolean CreateSequence( String[] Questions, String[] Interviewees,
	 * String Captions ){
	 * 
	 * P.PrintLn( P.Locator, "Creating a Sequence " );
	 * 
	 * if( MediaArray == null ){ MediaArray = new ArrayList(); } else{
	 * MediaArray.clear(); P.PrintLn( P.Log1, "MediaArray cleared" ); }
	 * 
	 * P.PrintLn( P.Debug1, "Captions: " + Captions );
	 * 
	 * if( Interviewees == null || Interviewees.length == 0 || Questions == null
	 * || Questions.length == 0 ){ } else{ // We read all the interviews with
	 * Interviewees[] and Questions[] for( int part = 0; part <
	 * Interviewees.length; part++ ){ for( int ques = 0; ques <
	 * Questions.length; ques++ ){
	 * 
	 * String queryString = "select Interview, QuestDesc, PartDesc " + "from " +
	 ...
	 * 
	 * // FIX ME // MediaItems = theDataBuilder.ReadMedia( Results.getValue( i,
	 * 0 ).toString(), null, null, null );
	 * 
	 * MediaItems.AddDescription( Results.getValue( i, 2 ).toString() + "\n" +
	 * Results.getValue( i, 1 ).toString(), true ); MediaArray.add( MediaItems
	 * );
	 * 
	 * } //} catch( IOException e ){ // P.PrintLn( P.Err,
	 * "Error retrieving Sequences " + e.toString() ); //} catch( Exception e ){
	 * // P.PrintLn( P.Err, "Error retrieving Media " + e.toString() ); //} } }
	 * } try{ if( MediaArray.size() > 0 ){ if( theSMILMedia.DoMontage(
	 * MediaArray ) ){
	 * 
	 * theSMILMedia.CreateSMILOutput( Captions.equals( "on" ) ); return true; }
	 * return false; } } catch( Exception e ){ P.PrintLn( P.Err,
	 * "Error retrieving Media " + e.toString() ); }
	 * 
	 * return false; }
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (repository.equals("VJ")) {
				...
				String[] VJ = { "http://www.cwi.nl/~media/ns/VP/VJ.rdf#VJ_Instance_20037" };

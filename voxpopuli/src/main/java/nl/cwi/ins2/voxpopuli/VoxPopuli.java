...
public class VoxPopuli {
	...
	private class LinkDescription {
		...
		public boolean equals(LinkDescription a) {
			...
			return SubjectChange.equals(a.SubjectChange) && ObjectChange.equals(a.ObjectChange)
					&& ModifierChange.equals(a.ModifierChange) && PredicateChange.equals(a.PredicateChange);
	...
	private class ArgumentationStructure {
		...
		/*
		 * ArgumentationStructure(ArgumentationStructure a){ this.ToulminType =
		 * a.ToulminType; this.AllowedLinks = new
		 * RhetType[a.AllowedLinks.length]; for( int i=0;
		 * i<a.AllowedLinks.length; i++ ){ this.AllowedLinks[i] =
		 * a.AllowedLinks[i]; } this.ReplaceInterviewee = a.ReplaceInterviewee;
		 * }
		 */
	}
	...
	private class MyArrayList extends ArrayList {
		...
		public boolean contains(RhetType a) {
			for (int i = 0; i < this.size(); i++) {
				LinkDescription b = (LinkDescription) this.get(i);
				if (b.SubjectChange.equals(a) || b.ModifierChange.equals(a) || b.PredicateChange.equals(a)
	...
	private class VerbalStatement {
		...
		/*
		 * String Object; String ObjectDescription;
		 */
		...
		VerbalStatement(VerbalStatement a) throws Exception {
			if (a == null) {
				...
				/*
				 * if( a.Object != null ){ this.Object = new String( a.Object );
				 * } if( a.ObjectDescription != null ){ this.ObjectDescription =
				 * new String( a.ObjectDescription ); }
				 */
				if (a.Claimer != null) {
	...
	static String RealTextServer = "http://media.cwi.nl:8080/cocoon/cuypers/oai/text?Content=";
	...
	// Order is important !!!!!!
	private final RhetType[] RhetActStrings = { new RhetType("Attack", ATTACK), new RhetType("Support", SUPPORT),
			new RhetType("Generalize", GENERALIZE), new RhetType("Associate", ASSOCIATE), new RhetType("Self", SELF),
			new RhetType("Specialize", SPECIALIZE) };

	// Order is important !!!!!!
	private final RhetType[] ToulminStrings = { new RhetType("Claim", CLAIM), new RhetType("Backing", BACKING),
	...
	private String[] ContrTopicArray = null; // List of possible controversial
												// topics

	private Question[] QuestionArray = null; // List of interviews with topic
												// and position

	private Interview[] InterviewArray = null; // List of interviews with topic
												// and position
	...
	private Hashtable AnswerConceptArray = null; // List of Concepts supporting
													// or opposing
	...
	public static void main(String[] args) {
		...
		try {
			...
			if ((args[5].equals("L") && args.length != 7) || (args[5].equals("M") && args.length != 11)
					|| (args[5].equals("C") && args.length != 7) || (args[5].equals("A") && args.length != 8)) {
				Usage();
			}
			 ...
			 * should be // able to choose one. For the moment we pick the first
			 * since there is only // one, but there must be a user interface
			 * here.
			 * 
			 * voxpopuli.DummySelectTopic();
			 * voxpopuli.GetInterviewByTopics(voxpopuli.ContrTopicArray[0]); //
			 * We retrieved all interviews with the topic, now there must be a
			 * selection // of the one we want to use. We call
			 * DummySelectInterviews() voxpopuli.DummySelectInterviews();
			 * 
	...
	private boolean Multiplevoices(String theArg, boolean keep, boolean NodeOn, boolean original, String filename)
			throws Exception {

		int ReplacedRoles = 0, max = 0, index[] = { -1, -1 };
		...
		for (int i = 0; i < Array.getLength(InterviewArray); i++) {
			if (InterviewArray[i].Arguments != null) {
				for (int ii = 0; ii < InterviewArray[i].Arguments.length; ii++) {
					// The following function looks whether the statements
					// contained in the
					// Argument can be replaced and constructs a new argument
					// with the replaced
					// statements.
					Temp = new Argument(InterviewArray[i].Arguments[ii]);
					ReplacedRoles = ReplaceableRoles(InterviewArray[i].Arguments[ii], true, theArgumentation, Temp,
							keep, NodeOn);
			...
			for (int i = 0; i < anArgument.Nodes.length; i++) {
				...
				if (aNode.theStatements != null) {
					...
					for (int ii = 0; ii < aNode.theStatements.length; ii++) {
						// This statement is the one contained in the
						// Argumentation
						...
						if (aStatement.ConnectedStatements != null) {
							...
							for (int j = 0; j < aStatement.ConnectedStatements.size(); j++) {
								...
								// This statement is the one linked to the one
								// contained in the Argumentation
								VerbalStatement bStatement = (VerbalStatement) Statements.get(aLink.StatementId);
								boolean present = false;
								for (int index1 = 0; !present && (index1 < Replaced.Nodes.length); index1++) {
									for (int index2 = 0; !present
											&& (index2 < Replaced.Nodes[index1].theStatements.length); index2++) {
								...
								// Only do filtering if flag is true, otherwise
								// count the statement
								// as replaceable
								if (filter == true) {

									// If the claimer is the same as the one of
									// the main statement, do not count it
									// if flag is true
									...
									if (expunge == true) {
										// This continue skips the increment
										// below
										continue;
									} else {
										...
										Temp = new VerbalStatement[Replaced.Nodes[i].theStatements.length];

										for (int v = 0; v < Replaced.Nodes[i].theStatements.length; v++) {
											// We copy the content before
											// overwriting it.
											Temp[v] = new VerbalStatement(Replaced.Nodes[i].theStatements[v]);
										...
										// Reallocate the array to make place
										// for the new statement
										Replaced.Nodes[i].theStatements = new VerbalStatement[Replaced.Nodes[i].theStatements.length
												+ 1];

										// We keep the original and add the
										// replacement
										int gap = 0;
										for (int v = 0; v < Replaced.Nodes[i].theStatements.length; v++) {
											// Add the statement after the
											// original one
											if ((keep && (v == (index + 1)))
													|| (!keep && (v == Replaced.Nodes[i].theStatements.length - 1))) {
												Replaced.Nodes[i].theStatements[v] = new VerbalStatement(bStatement);
												gap = 1;
											} else if (keep
													|| ((!keep) && (v != Replaced.Nodes[i].theStatements.length - 1))) {
												Replaced.Nodes[i].theStatements[v] = new VerbalStatement(Temp[v - gap]);
											} else {
	...
	// This function creates an ordered sequence of segments so that segments
	// are
	...
	private boolean CreateSegmentArray(ArrayList StatementsChain, boolean key, boolean clear) throws Exception {
		...
		for (int i = 0; i < StatementsChain.size(); i++) {
			if (key == true) {
				if ((aStatement = (VerbalStatement) Statements
						.get(((Link) StatementsChain.get(i)).StatementId)) == null) {
	 ...
	 * new ObjectOutputStream(file); OUTStream.writeObject(Statements);
	...
	private void SetDataStructure() throws Exception {
		...
		Weights = new WeightClass[] { new WeightClass("veryImportantFactor", 3), new WeightClass("importantFactor", 2),
		...
		StrictSupport = new ArgumentationStructure[] {
				new ArgumentationStructure(ToulminStrings[CLAIM],
						new RhetType[] { RhetActStrings[ATTACK], RhetActStrings[GENERALIZE], RhetActStrings[SPECIALIZE],
								RhetActStrings[ASSOCIATE] },
						true),
				new ArgumentationStructure(ToulminStrings[BACKING],
						new RhetType[] { RhetActStrings[ATTACK], RhetActStrings[GENERALIZE], RhetActStrings[SPECIALIZE],
								RhetActStrings[ASSOCIATE] },
						true),
				new ArgumentationStructure(ToulminStrings[CONCESSION],
						new RhetType[] { RhetActStrings[SUPPORT], RhetActStrings[SELF], RhetActStrings[GENERALIZE],
								RhetActStrings[SPECIALIZE], RhetActStrings[ASSOCIATE] },
						true),
				new ArgumentationStructure(ToulminStrings[CONDITION],
						new RhetType[] { RhetActStrings[SUPPORT], RhetActStrings[SELF], RhetActStrings[GENERALIZE],
								RhetActStrings[SPECIALIZE], RhetActStrings[ASSOCIATE] },
						true),
				new ArgumentationStructure(ToulminStrings[DATA],
						new RhetType[] { RhetActStrings[ATTACK], RhetActStrings[GENERALIZE], RhetActStrings[SPECIALIZE],
								RhetActStrings[ASSOCIATE] },
						true),
				new ArgumentationStructure(ToulminStrings[WARRANT],
						new RhetType[] { RhetActStrings[ATTACK], RhetActStrings[GENERALIZE], RhetActStrings[SPECIALIZE],
								RhetActStrings[ASSOCIATE] },
						true),
				new ArgumentationStructure(ToulminStrings[EXAMPLE], new RhetType[] { RhetActStrings[ATTACK],
						RhetActStrings[GENERALIZE], RhetActStrings[SPECIALIZE], RhetActStrings[ASSOCIATE] }, true) };

		GeneralSupport = new ArgumentationStructure[] {
				new ArgumentationStructure(ToulminStrings[CLAIM],
						new RhetType[] { RhetActStrings[ATTACK], RhetActStrings[ASSOCIATE] }, false),
				new ArgumentationStructure(ToulminStrings[BACKING],
						new RhetType[] { RhetActStrings[ATTACK], RhetActStrings[ASSOCIATE] }, false),
				new ArgumentationStructure(ToulminStrings[CONCESSION],
						new RhetType[] { RhetActStrings[SUPPORT], RhetActStrings[ASSOCIATE], RhetActStrings[SELF] },
						false),
				new ArgumentationStructure(ToulminStrings[CONDITION],
						new RhetType[] { RhetActStrings[SUPPORT], RhetActStrings[ASSOCIATE], RhetActStrings[SELF] },
						false),
				...
				new ArgumentationStructure(ToulminStrings[WARRANT],
						new RhetType[] { RhetActStrings[ATTACK], RhetActStrings[ASSOCIATE] }, false),
				new ArgumentationStructure(ToulminStrings[EXAMPLE],
						new RhetType[] { RhetActStrings[ATTACK], RhetActStrings[ASSOCIATE] }, false) };
		...
		StrictAttack = new ArgumentationStructure[] {
				...
				new ArgumentationStructure(ToulminStrings[WARRANT],
						new RhetType[] { RhetActStrings[SUPPORT], RhetActStrings[SELF], RhetActStrings[GENERALIZE],
								RhetActStrings[SPECIALIZE], RhetActStrings[ASSOCIATE] },
						true),
				new ArgumentationStructure(
						ToulminStrings[EXAMPLE], new RhetType[] { RhetActStrings[SUPPORT], RhetActStrings[SELF],
								RhetActStrings[GENERALIZE], RhetActStrings[SPECIALIZE], RhetActStrings[ASSOCIATE] },
						true) };

		GeneralAttack = new ArgumentationStructure[] {
				new ArgumentationStructure(ToulminStrings[CLAIM],
						new RhetType[] { RhetActStrings[SUPPORT], RhetActStrings[ASSOCIATE], RhetActStrings[GENERALIZE],
								RhetActStrings[SPECIALIZE], RhetActStrings[SELF] },
						false),
				new ArgumentationStructure(ToulminStrings[BACKING],
						new RhetType[] { RhetActStrings[SUPPORT], RhetActStrings[ASSOCIATE], RhetActStrings[GENERALIZE],
								RhetActStrings[SPECIALIZE], RhetActStrings[SELF] },
	...
	private boolean CreateSMILOutput(String OutFilename) throws Exception {
		...
		OutFile.print("<?xml version=\"1.0\" encoding=\"iso-8859-1\" standalone=\"yes\"?>\n"
				...
				+ (RealExt == true ? "xmlns:rn=\"http://features.real.com/2001/SMIL20/Extensions\">\n" : "")
				+ "<head>\n" + "<layout>\n" + "<root-layout width=\"" + Width + "\" height=\"" + (Height + STHeight)
				+ "\"/>\n" + "<region id=\"video-region\" left=\"0\" top=\"0\" width=\"" + Width + "\" height=\""
				+ Height + "\"/>\n" + "<region id=\"title-region\" left=\"" + (Width - TitleWidth) / 2 + "\" top=\""
				+ (Height - TitleHeight) / 2 + "\" width=\"" + TitleWidth + "\" height=\"" + TitleHeight + "\"/>\n"
				+ "<region id=\"subtitle-region\" left=\"0\" top=\"" + Height + "\" width=\"" + Width + "\" height=\""
				+ STHeight + "\"/>\n" + "</layout>\n" + "<transition id=\"" + TransName + "\" type=\"" + TransType
				+ "\"" + (!(TransSubtype.equals("")) ? " subtype=\"" + TransSubtype + "\"" : "") + " dur=\"" + TransDur
				+ "\"" + " startProgress=\"" + StartProgress + "\"" + " endProgress=\"" + EndProgress + "\""
				+ " direction=\"" + Direction + "\"" + "/>" + "</head>\n" + "<body>\n" + "<seq>\n");

		// The title (workaround RealOne problem
		if (TitleFileName != null) {
			OutFile.print("<video region=\"title-region\" src=\"" + VideoLocation + TitleFileName + "\" "
					+ "clipBegin=\"" + TitleBeginFrame + "\" " + "clipEnd=\"" + TitleEndFrame + "\" " + "clip-begin=\""
					+ TitleBeginFrame + "\" " + "clip-end=\"" + TitleEndFrame + "\" "
	...
	private boolean CreateSegmentSequence(PrintWriter OutFile) {
		...
		for (int i = 0; i < SegmentArray.size(); i++) {
			...
			// OutFile.print("<text id=\"subtitle" + i + "\"
			// region=\"subtitle-region\" begin=\"0\" src=\"" +
			OutFile.print("<text region=\"subtitle-region\" src=\"" + RealTextServer
					+ ((VideoSegment) SegmentArray.get(i)).Description.replaceAll(" ", "%20") + "&amp;width=" + Width
					+ "&amp;height=" + STHeight + "&amp;duration=" + STDuration + "&amp;fontsize=" + FontSize
					+ "&amp;fgcolor=%23" + FgColor + "&amp;bgcolor=%23" + BgColor + "&amp;text-align=" + TextAlign
					+ "&amp;ext=.rt\" " + "begin=\"0\" dur=\"" + STDuration + "\" "
			...
			// OutFile.print("<video id=\"video" + i + "\" region=\"video\"
			// src=\"" + aSegment.Filename +
			OutFile.print("<video region=\"video-region\" src=\"" + VideoLocation
					+ ((VideoSegment) SegmentArray.get(i)).FileName + Extension + "\" " + "clipBegin=\""
					+ ((VideoSegment) SegmentArray.get(i)).BeginFrame + "\" " + "clipEnd=\""
					+ ((VideoSegment) SegmentArray.get(i)).EndFrame + "\" " + "clip-begin=\""
					+ ((VideoSegment) SegmentArray.get(i)).BeginFrame + "\" " + "clip-end=\""
					+ ((VideoSegment) SegmentArray.get(i)).EndFrame + "\" "
					+ (((VideoSegment) SegmentArray.get(i)).TransIn != null
							? "transIn=\"" + ((VideoSegment) SegmentArray.get(i)).TransIn + "\" " : "")
					+ (((VideoSegment) SegmentArray.get(i)).TransOut != null
							? "transOut=\"" + ((VideoSegment) SegmentArray.get(i)).TransOut + "\" " : "")
	...
	private boolean CreateSegmentMontage(PrintWriter OutFile) {
		...
		for (Enumeration eSegment = SegmentImagesArray.elements(); eSegment.hasMoreElements();) {
			...
			// OutFile.print("<text id=\"subtitle" + i + "\"
			// region=\"subtitle-region\" begin=\"0\" src=\"" +
			OutFile.print("<text region=\"subtitle-region\" begin=\"0\" src=\"" + RealTextServer
					+ aSegment.Description.replaceAll(" ", "%20") + "&amp;width=" + Width + "&amp;height=" + STHeight
					+ "&amp;duration=" + STDuration + "&amp;fontsize=" + FontSize + "&amp;fgcolor=%23" + FgColor
					+ "&amp;bgcolor=%23" + BgColor + "&amp;text-align=" + TextAlign + "&amp;ext=.rt\" " + "transIn=\""
					+ aSegment.TransIn + "\" " + "transOut=\"" + aSegment.TransOut + "\" " + "dur=\"" + STDuration
					+ "\">\n" + "<param name=\"vAlign\" value=\"" + TextAlign + "\"/>\n" + "</text>\n");

			// OutFile.print("<video id=\"video" + i + "\" region=\"video\"
			// src=\"" + aSegment.Filename +
			OutFile.print("<video region=\"video-region\" src=\"" + VideoLocation + aSegment.FileName + "\" "
					+ "clipBegin=\"" + aSegment.BeginFrame + "\" " + "clipEnd=\"" + aSegment.EndFrame + "\" "
					+ "clip-begin=\"" + aSegment.BeginFrame + "\" " + "clip-end=\"" + aSegment.EndFrame + "\" "
					+ "transIn=\"" + aSegment.TransIn + "\" " + "transOut=\"" + aSegment.TransOut + "\" "
					+ (aSegment.ImageArray.size() > 0 ? "fill=\"transition\" " : "") + " />\n");

			if (aSegment.ImageArray != null) {
				for (Enumeration eImage = aSegment.ImageArray.elements(); eImage.hasMoreElements();) {
					Image theImage = (Image) eImage.nextElement();
					String ImageDuration = TimeDifference(theImage.EndFrame, theImage.BeginFrame);

					String ImageStart = TimeDifference(theImage.BeginFrame, aSegment.BeginFrame);

					// OutFile.print("<img id=\"img" + (z + i*z) + "\"
					// region=\"video\" src=\"" +
					OutFile.print("<img region=\"video\" src=\"" + PictureLocation + theImage.FileName + "\" "
							+ "begin=\"" + ImageStart + "s\" " + "dur=\"" + ImageDuration + "s\" " + "transIn=\""
							+ theImage.TransIn + "\" " + "transOut=\"" + theImage.TransOut + "\" " + " />\n");
					/*
	...
	private String ConvertToDate(long theDSec) {
		...
		String SDate = ((Hours < 10) ? "0" + Hours : "" + Hours) + ":" + ((Mins < 10) ? "0" + Mins : "" + Mins) + ":"
	...
	private boolean ReadTopics() {
		...
		try {
			...
			ContrTopicArray = new String[Results.size()];
	...
	private boolean RetrieveQuestions() {
		...
		try {
			...
			QuestionArray = new Question[Results.size()];
	...
	private boolean GetInterviewByTopics(String Topic) {
		...
		try {
			...
			InterviewArray = new Interview[Results.size()];

			for (int i = 0; i < Results.size(); i++) {
				Interview aSelectedInterview = new Interview();
				aSelectedInterview.Id = Results.get(i).getValue("Interview").toString();
				aSelectedInterview.Position = Results.get(i).getValue("ProCon").toString();
				aSelectedInterview.Topic = Topic;
	...
	private boolean GetAllInterviews() {
		...
		String queryString = "select Interview, ProCon, Topic, Question, QuestionText, Interviewee, Description "
				+ "from " + "{Interview} <serql:directType> {<VoxPopuli:InterviewSegment>}; " +
		...
		try {
			...
			for (int i = 0; i < Results.size(); i++) {
				Interview aSelectedInterview = new Interview();
				aSelectedInterview.Id = new String(Results.get(i).getValue("Interview").toString());
				aSelectedInterview.Position = new String(
						Results.get(i).getValue("ProCon") != null ? Results.get(i).getValue("ProCon").toString() : "");
				aSelectedInterview.Topic = new String(
						Results.get(i).getValue("Topic") != null ? Results.get(i).getValue("Topic").toString() : "");
				aSelectedInterview.thequestion = new Question(
						(Results.get(i).getValue("Question") != null ? Results.get(i).getValue("Question").toString()
								: ""),
						(Results.get(i).getValue("QuestionText") != null
								? Results.get(i).getValue("QuestionText").toString() : ""));

				aSelectedInterview.Interviewee = new Partecipant();
				aSelectedInterview.Interviewee.Id = new String(Results.get(i).getValue("Interviewee").toString());
	...
	private boolean ProcessInterview(int position) throws Exception {
		boolean result = false;

		// Descend in the interview statements
		Interview CurrentInterview = InterviewArray[position];

		// The following function is recursive and that is because the data
		// structure is
		// also recursive after a certain point; this is why we first have to
		// call this function
	...
	private int GetMaxLength(VerbalStatement aStatement, ArrayList current) throws Exception {
		...
		if (aStatement.ConnectedStatements != null) {
			int length = 0;
			// The Hashtable is used to explore the chain and gets every time
			// filled in
			// with the iterative exploration, which if it yields a max length,
			// gets copied
	...
	private boolean TraverseInterviewStatememts(Argument[] Arguments) throws Exception {
				...
				if (Arguments[i].Nodes != null) {
					for (int ii = 0; ii < Arguments[i].Nodes.length; ii++) {
						if (Arguments[i].Nodes[ii].theStatements != null) {
							for (int iii = 0; iii < Arguments[i].Nodes[ii].theStatements.length; iii++) {
								// Processing part
	...
	private boolean ConnectStatements(VerbalStatement theStatement) throws Exception {
		...
		boolean result = false;
		RhetType[] actions = { RhetActStrings[SUPPORT],
				// RhetActStrings[ASSOCIATE],
				RhetActStrings[GENERALIZE], RhetActStrings[SPECIALIZE], RhetActStrings[ATTACK] };
		...
		while (max > curmax && counter < 2) {
			...
			for (int i = 0; i < actions.length; i++) {
				...
				// Augment the set of statements that can be contrasted or
				// supported by
	...
	private boolean LinkStatements(VerbalStatement theStatement, MyArrayList StatementArray) throws Exception {
		// here we do the actual linking, this has become a separate function so
		// that
		// it is not needed to repeat the code twice

		VerbalStatement StatementToLink;

		// The hash table contains statements that only have the s/m/p/o filled
		// in
		...
		for (int i = 0; i < StatementArray.size(); i++) {
			...
			int TotalAND = (aStatement.Subject.equals("") ? 1 : 0) + (aStatement.Modifier.equals("") ? 1 : 0)
					+ (aStatement.Predicate.equals("") ? 1
							: 0) /*
									 * + ( aStatement.Object.equals( "" ) ? 1 :
									 * 0 )
									 */
			;
			...
			String whereclause;
			if (TotalAND > 0) {
				whereclause = " where " + (aStatement.Subject.equals("") ? "Subject = NULL " : "")
			...
			String queryString = "select Id  " + "from " + "{Id} <rdf:type> {<VoxPopuli:VerbalStatement>} "
					+ (!aStatement.Subject.equals("") ? ", {Id} <VoxPopuli:subject>  {<!" + aStatement.Subject + ">}"
							: ", [{Id} <VideoGen:subject> {Subject}]")
					+ (!aStatement.Modifier.equals("") ? ", {Id} <VoxPopuli:modifier> {<!" + aStatement.Modifier + ">}"
			...
			try {
				...
				for (int j = 0; j < Results.size(); j++) {
					// Apparently such a statement exists, so it MUST be in the
					// hash table
					// StatementToLink will be a fully filled in statement
					StatementToLink = null;
					if ((StatementToLink = (VerbalStatement) Statements
							.get(Results.get(j).getValue("Id").toString())) == null) {
						throw new Exception(
								"Missing statement in Hash Table " + Results.get(j).getValue("Id").toString());
					}
	...
	private boolean OneStepStatementManipulation(MyArrayList INStatements, int action) throws Exception {
			...
			for (int i = 0; i < size; i++) {
				...
				if (!theStatement.Subject.equals("")) {
					...
					for (int j = 0; j < limit; j++) {
						...
						aStatement.Subject = new String(Results.get(j).getValue("Subject") != null
								? Results.get(j).getValue("Subject").toString() : "");
				...
				if (theStatement.Modifier.equals("")) {
					...
					for (int j = 0; j < limit; j++) {
						...
						if (Results.get(j).getValue("Modifier") != null) {
							aStatement = new ConstructedStatement(theStatement);

							aStatement.LinkType = new MyArrayList(theStatement.LinkType);
							aStatement.LinkType.add(new LinkDescription(null, RhetActStrings[action], null, null));

							aStatement.Modifier = Results.get(j).getValue("Modifier").toString();
							aStatement.ModifierDescription = Results.get(j).getValue("ModifierDescription").toString();

							// This should avoid duplications of equal
							// statements
							if (!INStatements.contains(aStatement)) {
								INStatements.add(aStatement);
							}
				...
				if (!theStatement.Predicate.equals("")) {
					...
					for (int j = 0; j < limit; j++) {
						...
						aStatement.Predicate = new String(Results.get(j).getValue("Predicate") != null
								? Results.get(j).getValue("Predicate").toString() : "");
				...
				/*
				 * if( !theStatement.Object.equals( "" ) ){ String queryString =
				 * "select Object, ObjectDescription " + "from " + "{<!" +
				 * theStatement.Object + ">} " + Relation + " {Object}; " +
				 * Relation +
				 ...
				 * 
				 * aStatement.LinkType = new MyArrayList( theStatement.LinkType
				 * ); aStatement.LinkType.add( new LinkDescription(null, null,
				 * null, RhetActStrings[action]) );
				 * 
				 * aStatement.Object = new String( Results.get().getValue( j, 0
				 * ) != null ? Results.get().getValue( j, 0 ).toString() : "" );
				 * aStatement.ObjectDescription = new String(
				 * Results.get().getValue( j, 1 ) != null ?
				 * Results.get().getValue( j, 1 ).toString() : "" );
				 * 
				 ...
				 * aStatement.Predicate + aStatement.Object, aStatement );
				 * 
				 * } }
				 */
			}
	...
	private boolean ReadInterviewData(int position) throws Exception {
		...
		if (((InterviewArray[position].Segments = ReadMedia(InterviewArray[position].Id,
				"Q: " + InterviewArray[position].thequestion.QuestionText)) != null)
				&& ((InterviewArray[position].Arguments = ReadRhetoric(InterviewArray[position].Id, "rhetoricalForm",
						position)) != null)) {
			result = true;
		}
		return result;
	...
	private VideoSegment[] ReadMedia(String Id, String Description) throws Exception {
		...
		for (int i = 0; i < l; i++) {
			...
			theVideoSegments[i].FileName = new String(Results.get(i).getValue("Src").toString());
			theVideoSegments[i].BeginFrame = new String(Results.get(i).getValue("BeginFrame").toString());
			theVideoSegments[i].EndFrame = new String(Results.get(i).getValue("EndFrame").toString());
			theVideoSegments[i].Description = new String(Results.get(i).getValue("MediaDescription") != null
					? Results.get(i).getValue("MediaDescription").toString() + Description : Description);

			theVideoSegments[i].Language = new String(
					Results.get(i).getValue("Language") != null ? Results.get(i).getValue("Language").toString() : "");
			theVideoSegments[i].Quality = new String(
					Results.get(i).getValue("Quality") != null ? Results.get(i).getValue("Quality").toString() : "");
		}
	...
	private ToulminNode[] ReadToulminData(String Id, int position) throws Exception {
		...
		if (l != 0) {
			...
			for (int i = 0; i < l; i++) {
				...
				String Temp = NodeResults.get(i).getValue("NodeType").toString();
				...
				for (int ii = 0; ii < ll; ii++) {
					...
					Nodes[i].theStatements[ii] = ReadStatements(
							StatementsResults.get(ii).getValue("Statements").toString(), Nodes[i]);

					// Put the statement in the hash table to retrieve it more
					// easily
					if (Statements == null) {
						Statements = new Hashtable();
					}
					Statements.put(Nodes[i].theStatements[ii].Id, Nodes[i].theStatements[ii]);
				}
				...
				if (NodeDataResults.size() != 0) {
					...
					if (NodeDataResults.get(0).getValue("LogicType") != null) {
						Nodes[i].LogicType = NodeDataResults.get(0).getValue("LogicType").toString();
					} else {
						Nodes[i].LogicType = "";
					}
				}
	...
	private VerbalStatement ReadStatements(String Id, ToulminNode Node) throws Exception {

		VerbalStatement aStatement = null;

		// This function can be reused whenever statements need to be read

		String queryString = "select Subject, Modifier, Predicate, "
				+ "SubjectDescription, PredicateDescription, ModifierDescription, Explicit, "
				+ "Interviewee, Description " + "from " + "{<!" + Id + ">} " + " <VoxPopuli:subject> {Subject}; "
				+ "<VoxPopuli:explicit> {Explicit}; " + "<VoxPopuli:claimer> {Interviewee}; "
				+ "[<VoxPopuli:modifier> {Modifier}]; " + "[<VoxPopuli:predicate> {Predicate}]; "
		...
		try {
			...
			aStatement.Subject = Results.get(0).getValue("Subject") != null
					? Results.get(0).getValue("Subject").toString() : "";
			if (Results.get(0).getValue("Explicit") != null) {
				aStatement.Explicit = Results.get(0).getValue("Explicit").toString().equals("true") ? true : false;
			}

			// Set the parent node to know what kind of Toulmin node this
			// statement
			// belongs to
			aStatement.ParentNode = Node;

			aStatement.SubjectDescription = Results.get(0).getValue("SubjectDescription") != null
					? Results.get(0).getValue("SubjectDescription").toString() : "";
			aStatement.Modifier = Results.get(0).getValue("Modifier") != null
					? Results.get(0).getValue("Modifier").toString() : "";
			aStatement.ModifierDescription = Results.get(0).getValue("ModifierDescription") != null
					? Results.get(0).getValue("ModifierDescription").toString() : "";
			aStatement.Predicate = Results.get(0).getValue("Predicate") != null
					? Results.get(0).getValue("Predicate").toString() : "";
			aStatement.PredicateDescription = Results.get(0).getValue("PredicateDescription") != null
					? Results.get(0).getValue("PredicateDescription").toString() : "";
			/*
			 * aStatement.Object = new String( Results.get().getValue( 0, 3 ) !=
			 * null ? Results.get().getValue( 0, 3 ).toString() : "" );
			 * aStatement.ObjectDescription = new String(
			 * Results.get().getValue( 0, 7 ) != null ? Results.get().getValue(
			 * 0, 7 ). toString() : "" );
			 */
			aStatement.Claimer = new Partecipant();
			aStatement.Claimer.Id = Results.get(0).getValue("Interviewee").toString();
			aStatement.Claimer.Description = Results.get(0).getValue("Description").toString();

			String Temp = "S: " + (!aStatement.SubjectDescription.equals("") ? aStatement.SubjectDescription + " " : "")
	...
	private boolean PathosProcessInterview(String UserType, int position, int attitude) throws Exception {
		boolean result = false;

		/*
		 * if (!PathosHandleStatement(UserType, attitude)) { throw new
		 * Exception("Error creating attack statements"); } if
		 * (!SelectImages(UserType, position)) { throw new Exception(
		 * "Error selecting video segments"); } if (!AssignImages()) { throw new
		 * Exception("Error assigning video segments"); }
		 */
		return result;
	}

	private boolean PathosHandleStatement(String UserTYpe, int attitude, VerbalStatement AttackedStatement) {
		...
		try {
			...
			if (!AttackedStatement.Subject.equals("")) {
				 ...
				 * 
				 * // Check whether we are linked to a negative (for the user)
				 * object queryString = "select Point, PointDescription " +
				 * "from " + "{} <rdf:type> {<VoxPopuli:UserModel>}; " +
				 * "<VoxPopuli:userType> {Value}; " +
				 * "<VoxPopuli:negativeReaction> {Point} <VoxPopuli:partDescription> {PointDescription} "
				 * + "where Value = \"" + UserTYpe + "\" and " + "Point = <!" +
				 ...
				 * QueryLanguage.SERQL, queryString );
				 * 
				 * if( Results.getRowCount() == 1 ){ link = -link; } }
				 */
				...
				queryString = "select Point, PointDescription " + "from " + "{} <rdf:type> {<VoxPopuli:UserModel>}; "
				...
				for (int j = 0; j < limit; j++) {
					...
					aImageConcept.BeginFrame = new String(AttackedStatement.Segments[0].BeginFrame);
					aImageConcept.EndFrame = new String(AttackedStatement.Segments[0].EndFrame);

					aImageConcept.Concept = new String(Results.get(j).getValue("Point") != null
							? Results.get(j).getValue("Point").toString() : "");
	...
	private int GiveEthosRating(String UserType, String Interviewee) {
		...
		try {
			for (int i = 0; i < 3; i++) {
				...
				Rating = Rating + Results.size() * Weights[i].Weight;
			}
	...
	private boolean SelectImages(String UserType, int NrInterview) {
		...
		try {
			...
			aSegment.BeginFrame = new String(Results.get(0).getValue("BeginFrame").toString());
			aSegment.EndFrame = new String(Results.get(0).getValue("EndFrame").toString());
			aSegment.FileName = new String(Results.get(0).getValue("FileName").toString());
			aSegment.Description = new String(Results.get(0).getValue("Description").toString());
			aSegment.TransIn = TransName;
			aSegment.TransOut = TransName;
			// assuming max 100 pictures
			aSegment.ImageArray = new Hashtable(100);

			for (Enumeration eImgCon = AnswerConceptArray.elements(); eImgCon.hasMoreElements();) {
				...
				for (int j = 0; j < Results.size(); j++) {
					...
					aImage.FileName = new String(Results.get(j).getValue("FileName").toString());
					aImage.Description = new String(Results.get(j).getValue("Description").toString());
	...
	private boolean AssignImages() {
		...
		for (Enumeration eSeg = SegmentImagesArray.elements(); eSeg.hasMoreElements();) {
			...
			// make a structure containing per segment (begin end frame) the
			// pictures
			// that can be associated. This structure is a double hash table
			// that is
			// indexed by begin-end frame and filename respectively; in this way
			// no duplicate
			// are allowed
			if (aSegment.ImageArray != null) {
				for (Enumeration eImg = aSegment.ImageArray.elements(); eImg.hasMoreElements();) {
					Image aImage = (Image) eImg.nextElement();
					Hashtable hImgtoCmp = (Hashtable) SubSegwithImg.get(aImage.BeginFrame + aImage.EndFrame);
				...
				while (stay) {
					...
					while (action) {
						...
						action = false;
						for (Enumeration eSubSeg = SubSegwithImg.elements(); eSubSeg.hasMoreElements();) {
							...
							if (ImginSeg.size() == 1) {
								...
								// This clears also the hash table in
								// SubSegwithImg, so
								// the get returns a pointer
								ImginSeg.clear();
								// Eliminate the picture from the rest of the
								// segments
					...
					for (Enumeration eSubSeg = SubSegwithImg.elements(); eSubSeg.hasMoreElements();) {
						// This loop assigns subsegments with more than one
						// picture
						Hashtable ImginSeg = (Hashtable) eSubSeg.nextElement();
						if (ImginSeg.size() > 0) {
							Enumeration eImg = ImginSeg.elements();
							Image theImage = (Image) eImg.nextElement();
							aSegment.ImageArray.put(theImage.BeginFrame, theImage);
							// This clears also the hash table in SubSegwithImg,
							// so
							// the get returns a pointer
							ImginSeg.clear();
							Eliminate(SubSegwithImg, theImage);
	...
	private boolean PrintConnectedStatement(MyArrayList StatementsArray, boolean key) {
		...
		// The key is used to understand if the Hashtable contains the
		// statements
		// or the keys to them
	...
	private boolean OrderArgument(Argument MultipleVoices, boolean do_order) throws Exception {
		...
		boolean result = false;
		int[] order = { EXAMPLE, DATA, WARRANT, BACKING, CLAIM, CONCESSION, CONDITION };
	...
	// This function finds checks that there are no paths between concepts that
	...
	private boolean CheckReciprocalLinks(String Concept, String ConceptDescription) throws Exception {
		...
		String[] Relation = { "logicalsimilar", "logicalopposite", "association", "generalization" };

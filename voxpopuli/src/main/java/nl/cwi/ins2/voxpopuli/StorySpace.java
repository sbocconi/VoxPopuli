...
public class StorySpace{
  ...
  public StorySpace( Hashtable Videos, Hashtable Audios, Hashtable Images, Hashtable Texts ) throws Exception{
    ...
    if( Audios != null ){
      for( Enumeration e = Audios.elements(); e.hasMoreElements(); ){
  ...
  public ArrayList GetLastPlot() throws Exception{
    ...
    int maxlength = u.max( size0, size1, size2 );
  ...
  public boolean AddUniquetoPlot( MediaItem a, int i ) throws Exception{
    ...
    if( a.VideoSegments.size() > 0 ||
        a.AudioSegments.size() > 0 ||
        a.StillSegments.size() > 0 ||
